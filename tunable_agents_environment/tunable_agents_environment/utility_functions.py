...
def linear_utility(reward: np.ndarray, weights: np.ndarray):
    return np.dot(reward, weights)


def polinomial_utility(reward: np.ndarray, coefficients: np.ndarray):
...
def threshold_utility(reward: np.ndarray, thresholds: np.ndarray):
